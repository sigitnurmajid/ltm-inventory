...
*/
  ...
  var helpers = {
    ...
    _deprecated: function (scope, value, previous, current) {
      if (value !== undefined) {
        console.warn(
          scope + ': "' + previous + '" is deprecated. Please use "' + current + '" instead'
        )
      }
    },
  }
  ...
  var core_animations = {
    ...
    advance: function () {
      ...
      while (i < animations.length) {
        ...
        nextStep =
          Math.floor(((Date.now() - animation.startTime) / animation.duration) * numSteps) + 1
  ...
  helpers$1.extend(DatasetController.prototype, {
    ...
    _resolveDataElementOptions: function (element, index) {
      ...
      if (helpers$1.isArray(elementOptions)) {
        ...
        for (i = 0, ilen = keys.length; i < ilen; ++i) {
          key = keys[i]
          values[key] = resolve(
            [custom[key], datasetOpts[elementOptions[key]], datasetOpts[key], options[key]],
            context,
            index,
            info
          )
    ...
    setHoverStyle: function (element) {
      ...
      model.borderColor = resolve(
        [custom.hoverBorderColor, dataset.hoverBorderColor, getHoverColor(model.borderColor)],
        undefined,
        index
      )
  ...
  var controller_bar = core_datasetController.extend({
    ...
    calculateBarValuePixels: function (datasetIndex, index, options) {
      ...
      var start =
        value.start === undefined ? 0 : value.max >= 0 && value.min >= 0 ? value.min : value.max
      var length =
  ...
  var controller_doughnut = core_datasetController.extend({
    ...
    update: function (reset) {
      ...
      if (circumference < DOUBLE_PI$1) {
        ...
        var contains90 =
          (startAngle <= HALF_PI$1 && endAngle >= HALF_PI$1) || endAngle >= DOUBLE_PI$1 + HALF_PI$1
    ...
    updateElement: function (arc, index, reset) {
      ...
      var circumference =
        reset && animationOpts.animateRotate
          ...
          : arc.hidden
          ? 0
          : me.calculateCircumference(dataset.data[index]) * (opts.circumference / DOUBLE_PI$1)
  ...
  var controller_line = core_datasetController.extend({
    ...
    _resolveDatasetElementOptions: function (element) {
      ...
      values.clip = toClip(
        valueOrDefault$6(config.clip, defaultClip(me._xScale, me._yScale, values.borderWidth))
      )
    ...
    /** @lends Chart */ {
      ...
      resize: function (silent) {
        ...
        var newHeight = Math.max(
          0,
          Math.floor(aspectRatio ? newWidth / aspectRatio : helpers$1.getMaximumHeight(canvas))
        )
      ...
      buildOrUpdateScales: function () {
        ...
        helpers$1.each(items, function (item) {
          ...
          if (
            positionIsHorizontal(scaleOptions.position) !== positionIsHorizontal(item.dposition)
          ) {
    ...
    fit: function () {
      ...
      if (tickOpts.display && display) {
        ...
        if (isHorizontal) {
          ...
          me.paddingLeft =
            Math.max(((paddingLeft - offsetLeft) * me.width) / (me.width - offsetLeft), 0) + 3
          me.paddingRight =
            Math.max(((paddingRight - offsetRight) * me.width) / (me.width - offsetRight), 0) + 3
        } else {
  ...
  var scale_radialLinear = scale_linearbase.extend({
    ...
    setReductions: function (largestPossibleRadius, furthestLimits, furthestAngles) {
      ...
      var radiusReductionBottom =
        -Math.max(furthestLimits.b - (me.height - me.paddingTop), 0) / Math.cos(furthestAngles.b)
  ...
  var scale_time = core_scale.extend({
    ...
    buildTicks: function () {
      ...
      me._majorUnit =
        !tickOpts.major.enabled || me._unit === 'year' ? undefined : determineMajorUnit(me._unit)
    ...
    fit: function () {
      ...
      if (isHorizontal) {
        ...
        helpers$1.each(me.legendItems, function (legendItem, i) {
          ...
          if (
            i === 0 ||
            lineWidths[lineWidths.length - 1] + width + 2 * labelOpts.padding > minSize.width
          ) {
